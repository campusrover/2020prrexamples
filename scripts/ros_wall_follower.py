...
# Small modifications to https://raw.githubusercontent.com/ssscassio/ros-wall-follower-2-wheeled-robot/master/catkin_ws/src/two-wheeled-robot-motion-planning/scripts/follow_wall.py
# The Lidar data is rotated by 180 degrees, and also the constants that assume 360*3 rays are adjusted
# what we get, which is 360 rays. However it still doesn't work!
...
def clbk_laser(msg):
    ...
    newrangelist = msg.ranges[157:] + msg.ranges[:156]
    msg.ranges = newrangelist
    size = len(msg.ranges)
    ...
    regions_ = {
        'bright':  min(min(msg.ranges[0:45]), inf),
        'right': min(min(msg.ranges[46:90]), inf),
        'fright':  min(min(msg.ranges[91:135]), inf),
        'front':  min(min(msg.ranges[136:180]), inf),
        'fleft':   min(min(msg.ranges[181:225]), inf),
        'left':   min(min(msg.ranges[226:270]), inf),
        'bleft':   min(min(msg.ranges[271:316]), inf),
    }
